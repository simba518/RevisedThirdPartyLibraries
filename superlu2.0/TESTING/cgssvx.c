...
void
cgssvx(char *fact, char *trans, char *refact,
       SuperMatrix *A, factor_param_t *factor_params, int *perm_c,
       int *perm_r, int *etree, char *equed, float *R, float *C,
 ...
 *         - diag_pivot_thresh (float): Diagonal pivoting threshold.
 ...
 *         L has types: Stype = SC, Dtype = SLU_C, Mtype = TRLU.
 *
...
#if 0
printf("cgssvx: fact=%c, trans=%c, refact=%c, equed=%c\n",
