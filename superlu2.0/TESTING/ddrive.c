 ...
 */
    ...
    extern int sp_dget02(char *, int, int, int, SuperMatrix *, double *,
                         int, double *, int, double *resid);
    extern int sp_dget04(int, int, double *, int, 
                         double *, int, double rcond, double *resid);
    extern int sp_dget07(char *, int, int, SuperMatrix *, double *, int,
		...
		*fact = facts[ifact];
		...
		for (irefact = 0; irefact < nrefact; ++irefact) {
		    *refact = refacts[irefact];
			...
			dgstrf("N", &AC, iparam.diag_pivot_thresh,
			...
			*trans = transs[itran];
			...
			if ( nofact && norefact && itran == 0) {
			    ...
			    dgssv(&A, perm_c, perm_r, &L, &U, &X, &info);
			...
			dgssvx(fact, trans, refact, &A, &iparam, perm_c,
